...
sampleName=["wzp6_ee_mumuH_Htautau_ecm240","wzp6_ee_mumuH_HWW_ecm240", "wzp6_ee_mumuH_Hbb_ecm240","wzp6_ee_mumuH_ecm240","wzp6_ee_qqH_ecm240","wzp6_ee_tautauH_ecm240","p8_ee_ZZ_ecm240","p8_ee_WW_ecm240"]
...
ROOT.gInterpreter.Declare("""
   ...
   ROOT::VecOps::RVec<int> FindBest(ROOT::VecOps::RVec<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzM4D<double>>> P4vector, double mass){ 
       ...
       int third=-1; double ptThird=0;
   
       for (int i=0; i<P4vector.size(); i++){
		if (i==dau1 || i==dau2) continue;
		if(P4vector[i].Pt()>ptThird) { ptThird=P4vector[i].Pt(); third=i;}
       }
...
#    aceptancia/efficiencia *solo* para sucesos HTauTau hadronico
    ...
    # muones 
    df[p] = df[p].Filter("NMuon == 3", "Events with at two muons")
    ...
    df[p] = df[p].Define("bestZ","FindBest(Muon_p4,91)")
    df[p] = df[p].Define("Lead","bestZ[0]")
    df[p] = df[p].Define("Second","bestZ[1]")
    df[p] = df[p].Define("HiggsMuon","bestZ[2]")
    df[p] = df[p].Filter("Muon_pt[Lead]>10&&Muon_pt[Second]>10","Muon Pt>10")
    df[p] = df[p].Filter("Muon_charge[Lead] != Muon_charge[Second]", "Muons with opposite charge")

    # Reconstruimos el Z! Sumando los cuadrimomentos (p4)
    df[p] = df[p].Define("DiMuon_p4","Muon_p4[Lead]+Muon_p4[Second]")
...
#    df[p] = df[p].Filter("recoil<100", "Recoil<100")
    ...
    df[p] = df[p].Filter("NTauFromJet == 1", "Events with exactly one tau")
    df[p] = df[p].Filter("NElectron == 0", "Events with no electrons")
    ...
    df[p] = df[p].Define("TauLead_p4","Taus_p4[0]")
    df[p] = df[p].Filter("TauFromJet_type[0]>=0", "Identified Tau")
    df[p] = df[p].Filter("TauFromJet_pt[0]>10", "Tau Pt>10 GeV")

    df[p] = df[p].Define("MuonHiggs_p4","Muon_p4[HiggsMuon]")
    df[p] = df[p].Filter("MuonHiggs_p4.Pt()>5", "Muon Pt>5 GeV")

    # Carga opuesta
    df[p] = df[p].Filter("TauFromJet_charge[0] != Electron_charge[0]", "Electron-Tau with opposite charge")
 
    # Reconstruimos el DiTau, Sumando los cuadrimomentos (p4)
    df[p] = df[p].Define("DiTau_p4","TauLead_p4+MuonHiggs_p4")

    df[p] = df[p].Define("TauLead_type","TauFromJet_type[0]")
    df[p] = df[p].Define("TauLead_mass","TauFromJet_mass[0]")
    ...
    df[p] = df[p].Define("p12","TauLead_p4.Py()*MuonHiggs_p4.Px()-TauLead_p4.Px()*MuonHiggs_p4.Py()")
    df[p] = df[p].Define("r0","abs((Missing_p4.Py()*TauLead_p4.Px()-Missing_p4.Px()*TauLead_p4.Py())/p12)")
    df[p] = df[p].Define("f0","1/(1+r0)")
    df[p] = df[p].Define("r1","abs((Missing_p4.Py()*MuonHiggs_p4.Px()-Missing_p4.Px()*MuonHiggs_p4.Py())/p12)")
    df[p] = df[p].Define("f1","1/(1+r1)")
    df[p] = df[p].Define("DiTau_coll_mass","DiTau_vis_mass/sqrt(f0*f1)")


    df[p] = df[p].Define("DThetaTaus","abs(TauLead_p4.Theta()-MuonHiggs_p4.Theta())")
    df[p] = df[p].Define("DPhiTaus","abs(ROOT::Math::VectorUtil::DeltaPhi(TauLead_p4,MuonHiggs_p4))")
    #df[p] = df[p].Filter("DPhiTaus>1.5","DPhiTaus>1.5")

    df[p] = df[p].Define("DThetaMuons","abs(Muon_p4[0].Theta()-Muon_p4[1].Theta())")
    df[p] = df[p].Define("DPhiMuons","ROOT::Math::VectorUtil::DeltaPhi(Muon_p4[0],Muon_p4[1])")
...
for i in range(nProcesses):
   ...
   hMuonHiggsPt[p] = df[p].Define("MuonHiggs_Pt","MuonHiggs_p4.Pt()").Histo1D(("MuonHiggs_Pt_{}".format(p), "Second Tau Pt; Second #tau Pt (GeV);N_{Events}",60,0,120), "MuonHiggs_Pt")
   hMuonHiggsTheta[p] = df[p].Define("MuonHiggs_Theta","MuonHiggs_p4.Theta()").Histo1D(("MuonHiggs_Theta_{}".format(p), "Second Tau #theta; Second #tau #theta;N_{Events}",50,0,3.16), "MuonHiggs_Theta")
   hMuonHiggsPhi[p] = df[p].Define("MuonHiggs_Phi","MuonHiggs_p4.Phi()").Histo1D(("MuonHiggs_Phi_{}".format(p), "Second Tau #phi; Second #tau #phi;N_{Events}",50,-3.16,3.16), "MuonHiggs_Phi")

   hLeadTauType[p] = df[p].Histo1D(("LeadTau_Type_{}".format(p), "Lead Tau Type; Lead #tau Type;N_{Events}",15,0, 15), "TauLead_type")
   hLeadTauMass[p] = df[p].Histo1D(("LeadTau_Mass_{}".format(p), "Lead Tau Mass; Lead #tau Mass;N_{Events}",100,0, 4), "TauLead_mass")
#   hLeadTauIso[p] = df[p].Histo1D(("LeadTau_Iso_{}".format(p), "Lead Tau Iso; Lead #tau Iso;N_{Events}",100,0, 2), "TauLead_iso")
#   hMuonHiggsIso[p] = df[p].Histo1D(("MuonHiggs_Iso_{}".format(p), "Second Tau Iso; Second #tau Iso;N_{Events}",100,0, 2), "MuonHiggs_iso")
   ...
   hDPhiTaus[p]  =  df[p].Histo1D( ("dPhiTaus_{}".format(p),"DPhiTaus;#Delta #phi Taus;N_{Events}",100,0,3.2),"DPhiTaus")
   hDPhiMuons[p]  =  df[p].Histo1D( ("dPhiMuons_{}".format(p),"DPhiMuons;#Delta #phi Muons;N_{Events}",100,0,3.2),"DPhiMuons")
   hDThetaTaus[p]  = df[p].Histo1D(("dThetaTaus_{}".format(p),"DThetaTaus;#Delta #Theta Taus;N_{Events}",100,0,6.28),"DThetaTaus")
   hDThetaMuons[p]  = df[p].Histo1D(("dThetaMuons_{}".format(p),"DThetaMuons;#Delta #Theta Muons;N_{Events}",100,0,6.28),"DThetaMuons")
...
for p in processes:
   ...
   hLeadTauPt[p].Write()
   hMuonHiggsPt[p].Write()
   hLeadTauTheta[p].Write()
   hMuonHiggsTheta[p].Write()
   hLeadTauPhi[p].Write()
   hMuonHiggsPhi[p].Write()
   hZPt[p].Write()
   hDiTauPt[p].Write()
   hRecoilPt[p].Write()
   hZRapidity[p].Write()
